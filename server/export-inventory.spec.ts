...
describe('Export Inventory CSV', () => {
  ...
  test('should return CSV with correct headers', async () => {
    ...
    await login(agent);
    const res = await agent.get('/api/export/inventory').set('Accept', 'text/csv');
    expect(res.status).toBe(200);
    expect(res.headers['content-type']).toContain('text/csv');
    expect(res.headers['content-disposition']).toContain('inventario');
    expect(res.text.split('\n')[0].toLowerCase()).toContain('product');
  });
});
